...
public class MyDialog {
                ...
                .setPositiveButton("确认", new OnClickListener() {
                    ...
                    public void onClick(DialogInterface arg0, int arg1) {
                        ...
                        if ((s5 = course_count.getText().toString()).equals("") || s1.equals("")) {
                            ...
                            for (int m = 0; m < i; m++) {
                                MainActivityClass.db.update(day, n + m + 1, s1, s2, s3, s4, s5, s6, s7, Integer.toString(m));
                            }
    ...
    */
    public void modify(final int day, final int n) {
        ...
        for (int i = 0; i < 8; i++) {
            temp[i] = main.cursor[day].getString(i + 1);
        }
        //将旧数据显示在编辑对话框
        if (!temp[0].equals("")) course_name.setText(temp[0].substring(temp[0].indexOf(":") + 2));
        if (!temp[1].equals(""))
            course_address.setText(temp[1].substring(temp[1].indexOf(":") + 2));
        if (!temp[2].equals(""))
            course_teacher.setText(temp[2].substring(temp[2].indexOf(":") + 2));
                    ...
                    public void onClick(DialogInterface arg0, int arg1) {
                        ...
                        if (s5.equals("") || n1 == Integer.parseInt(s5.trim())) {
                            switch (n2) {
                                ...
                                case 1:
                                    MainActivityClass.db.update(day, n, s1, s2, s3, s4, s5, s6, s7, "0");
                                    for (int m = 1; m < n1; m++) {
                                        MainActivityClass.db.update(day, n + m, s1, s2, s3, s4, s5, s6, s7, Integer.toString(m));
                                    }
                                    break;
                                case 2:
                                    MainActivityClass.db.update(day, n - 1, s1, s2, s3, s4, s5, s6, s7, "0");
                                    MainActivityClass.db.update(day, n, s1, s2, s3, s4, s5, s6, s7, "1");
                                    for (int m = 2; m < n1; m++) {
                                        MainActivityClass.db.update(day, n + m - 1, s1, s2, s3, s4, s5, s6, s7, Integer.toString(m));
                                    }
                                    break;
                                case 3:
                                    for (int m = n2; m >= 0; m--) {
                                        MainActivityClass.db.update(day, n - m + 1, s1, s2, s3, s4, s5, s6, s7, Integer.toString(n2 - m));
                                    }
                                    break;
                            }
                        ...
                        //若节数有变化，先确定新节数并赋予旧的数据再更新数据
                            ...
                            if (n3 < n1) {
                                switch (n2) {//删除
                                    case 0:
                                        for (int m = 0; m < n1; m++) {
                                            MainActivityClass.db.deleteData(day, n + m + 1);
                                        }
                                        break;

                                    case 1:
                                        MainActivityClass.db.deleteData(day, n);
                                        for (int m = 1; m < n1; m++) {
                                            MainActivityClass.db.deleteData(day, n + m);
                                        }
                                        break;
                                    case 2:
                                        MainActivityClass.db.deleteData(day, n - 1);
                                        MainActivityClass.db.deleteData(day, n);
                                        for (int m = 2; m < n1; m++) {
                                            MainActivityClass.db.deleteData(day, n + m - 1);
                                        }
                                ...
                                switch (n2) {//更新数据
                                    ...
                                    case 3:
                                        for (int m = 0; m < n3; m++) {
                                            MainActivityClass.db.update(day, n + m - 2, s1, s2, s3, s4, s5, s6, s7, Integer.toString(m));
                                        }
                                        break;
                                }
